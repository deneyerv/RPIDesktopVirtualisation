...
if [ $number -lt 4 ];then
	...
	do
        docker run -d -it  --name vnc_container$i  deneyervivien/rpidesktopvirtualisation-container-vnc sh
	sleep 20s
	...
	for i in `seq $i  $numberofclidocker`;
        do
	docker run -d -it  --name cli_container$i deneyervivien/rpidesktopvirtualisation-container-alpine  sh
        sleep 15s
	...
	for i in `seq $i $number`;
	do
        docker run  -d  -it --name cli_container$i -p 10.0.0.23:22:22 deneyervivien/rpidesktopvirtualisation-container-alpine  sh
        sleep 15s
        docker network connect local cli_container$i
	done
fi
